    ...
    const fetchScenario = async () => {
      try {
        ...
        const response = await api.get(
          `/api/modules/${moduleId}/scenarios/${scenarioId}`
        );
        setFormData(response.data);
      } catch (err: any) {
        console.error('Error fetching scenario:', err);
        // Store available scenarios if returned in error response
        if (err.response?.data?.availableScenarios) {
          setAvailableScenarios(err.response.data.availableScenarios);
        }
      ...
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          ...
          {availableScenarios.length > 0 && (
            <div className="mt-2">
              <p className="font-semibold">Available scenarios:</p>
